...
import {
  getChipByUid,
  getActiveRewardPeriod,
  hasChipClaimedReward,
  createClaim,
} from "@/lib/database"
  ...
  try {
    ...
    if (!signature) {
      return NextResponse.json(
        {
          error:
            "Missing signature. Please sign the message to verify wallet ownership.",
        },
        { status: 400 },
      )
    }
    ...
    if (!message) {
      return NextResponse.json(
        {
          error:
            "Missing message. Please sign the message to verify wallet ownership.",
        },
        { status: 400 },
      )
    }
    ...
    if (!chip) {
      return NextResponse.json(
        {
          error:
            "This hat is not authorized. Please contact the Department of Agriculture.",
        },
        { status: 403 },
      )
    }
    ...
    const hasClaimed = await hasChipClaimedReward(
      chip.id,
      activeRewardPeriod.id,
    )
