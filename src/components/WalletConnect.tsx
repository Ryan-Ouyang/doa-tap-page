...
interface WalletConnectProps {
  onSignatureComplete: (address: Address, message: string, signature: Hex) => void;
}
...
export function WalletConnect({ onSignatureComplete }: WalletConnectProps) {
  ...
  const handleSign = async () => {
    ...
    try {
      ...
      const siweMessage = createWalletVerificationMessage(address as `0x${string}`);
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          {connectors.map((connector) => (
            ...
            >
              {isConnectPending ? 'Connecting...' : `Connect with ${connector.name}`}
            </button>
          ))}
        </div>
      </div>
